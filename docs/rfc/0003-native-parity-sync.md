...
**同一天 macOS CI 那两次红不是它。** 拿到日志后是另一回事：`free_port()` 从内核的
自动分配池（macOS 49152–65535）里借一个端口号再还回去，窗口期内那个号被别的
`bind(0)` 抢走，`gld start` 于是报「端口已被占用」而占用者是另一个 gld。改成从
20000–32000 挑号——那一段不在任何系统的自动分配范围里。细节写在 `free_port` 的
注释里，不在这儿重复。
