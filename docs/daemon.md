...
守护进程每次读写配置前也会先重读这个文件，所以它跑着时手工改 `profiles.json` 也不会被冲掉；
只是已经在跑的服务还用着旧值，`gld restart` 之后才生效。
...
**从 0.7.0（协议 4）升到 0.8.0（协议 5）时**：什么都不用额外做。换完二进制、还没重启时，命令行报
"守护进程版本 0.7.0（协议 4）与命令行版本 0.8.0（协议 5）不一致"（退出码 4），`gld daemon restart`
之后就好了。（2026-09-24 从 main 自己编的构建版本号还写着 0.7.0，报的是"0.7.0（协议 4）与 0.7.0（协议 5）"，
一回事。）工具表的说明改过（`confirm` 参数），ChatGPT 要点一次 Refresh 才看得到，不点也照常能用，
见[安装 · 换完核对](install.md#换完核对)。

**从 0.6.0（协议 3）升到 0.7.0（协议 4）时**：什么都不用额外做。换完二进制、还没重启时，命令行
会报"守护进程版本 0.6.0（协议 3）与命令行版本 0.7.0（协议 4）不一致"（退出码 4），`gld daemon restart`
之后就好了；服务、项目、凭据、ChatGPT 的授权都不动。

**从 0.5.0（协议 2）升到 0.6.0（协议 3）时**：旧命令起的单项目服务会随
`gld daemon restart` 一起停掉、不再回来；在任意项目目录里 `gld start` 一次，服务起来，
登记过的项目都会加进去（输出里逐个点名）。客户端要改连服务的地址（`gld ls` 里那一行）。

版本号或协议号变了而没重启时，执行任何命令都会提示“守护进程版本 x 与命令行版本 y 不一致”（退出码 4），
`daemon status` / `daemon stop` 不受影响。两样都没变的新构建**不会**提示，旧代码照跑，
所以换完二进制一律重启，核对办法见[安装 · 换完核对](install.md#换完核对)。
