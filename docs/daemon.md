...
**从 0.7.0（协议 4）升到加了 grant 的构建（协议 5，版本号还是 0.7.0）时**：什么都不用额外做。换完二进制、
还没重启时，命令行报"守护进程版本 0.7.0（协议 4）与命令行版本 0.7.0（协议 5）不一致"（退出码 4），
`gld daemon restart` 之后就好了。工具表的说明改过（`confirm` 参数），ChatGPT 要点一次 Refresh 才看得到，
不点也照常能用，见[安装 · 换完核对](install.md#换完核对)。
