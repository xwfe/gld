...
自己的工具集、白名单、读限制照样生效，但拿服务口令或服务令牌进来的，能进项目表里的每一个。
要给别人或另一个客户端只开几个项目，发一把 grant，别把服务口令给出去，见
[concepts.md](concepts.md#只开几个项目gld-grant)。**只读的 grant（默认）才是真的挡得住的范围。**
`--write` 的能跑命令，而命令能以你的身份读到数据目录里的服务口令（下面说的那几道门只管文件类
工具），所以它等于全权。
