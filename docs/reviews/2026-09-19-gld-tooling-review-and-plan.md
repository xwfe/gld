...
日期：2026-09-19  
状态：**原始审查与后续实施记录；U0–U5 及 A01–A20 的历史验收见第 10–11 节，后续诊断补齐见第 12 节。**

目标：让其他大模型据此改进 gld 的可用性、数据安全和错误恢复，不通过放松全部权限来掩盖问题。

> 2026-09-23 对账：前半部分保留 9 月 19 日的故障与建议，不是当前未完成清单。
> 新发现、运行/客户端差异与下一步见[生命周期与文档审查](2026-09-23-lifecycle-and-docs-audit.md)。
...
> **X 编号撞车了，别读串。**下表的 X01–X03 是**本文自己的**编号（命令会话那一组）。
> 跨仓重构评审另有一套 X01–X13，含义完全不同——它的 X01 是 `toexec-fs` 在 Windows 上
> 先删后 rename，X02 是文件状态三态，X03 是 hub 的执行资源所有权。本文后面凡是指那一套的，
> 一律写成"跨仓 X0n"；不带前缀的就是下表这一套。两套编号都已经写进了提交记录，
> 改名只会让旧记录指向不存在的条目，所以留着编号、加前缀区分。
...
### U1 第二批：空 `only:` 与输出游标（C03、本文 X01）
...
| 本文 X01 `read_output` 越过 1 MiB 保留缓冲 | `offset` 按保留缓冲算、有没有下一页按累计字节算。实测 3 MiB 输出读到 `offset=1048576` 时回空内容、`next_offset` 还是 `1048576`——照着它再读就是死循环 | 偏移改成**整条流里的绝对位置**；读得到的那段是 `[retained_from, total)`，每页 `next_offset` 严格前进，读完就是 `None` |
| 本文 X01 附带：旧 offset 落在已被挤掉的区间 | 悄悄当成缓冲的第一个字节，报 `offset: 0`——把 2 MiB 之后的内容说成开头 | 报实际起点 `offset`、丢了多少 `dropped_bytes`，外加一条 `no longer retained` 警告 |
| 本文 X01 附带：分页接缝劈开多字节字符 | 每页接缝上多出替换字符 | 截到字符边界再返回，下一页从那里接着读（`limit` 小到装不下一个字符时照原样给，保证能前进） |
...
### U4 第二批：输出能读多久、stdin 怎么关（本文 X02、X03）

**输出寿命统一了（本文 X02、验收 A16）。**以前内联跑完的会话 30 秒后回收，转后台那条路的回收时点却挂在命令自己的 deadline 上——同样是"跑完了"，能读多久取决于当初怎么调的，而调用方没办法知道还剩多少。现在一律**从进程结束那一刻起算 5 分钟**，`read_output` 回 `expires_in_ms` 和 `retention_ms`。判据在 `SessionStore::sweep`（惰性，每次 get/insert 扫一遍），定时器只负责早点把内存还回去，睡过头也不影响正确性。
...
**stdin 有三种状态了（本文 X03、验收 A17）。**以前只有"给了就写完关掉"和"什么都不做"，而"什么都不做"意味着 stdin 一直开着却永远没有数据：`cat`、`grep foo` 会一路挂到 `timeout_ms`。现在 `stdin_mode` 有 `close`（没给 stdin 时的默认，起来就关，命令读到 EOF）、`once`（给了 stdin 时的默认）、`interactive`（留着，用 `write_stdin` 接着喂）。
...
### U5 收尾：把方案 D 里剩下的两条做掉

U0–U5 六个阶段之后，方案 D 还留着两句没落实。这一批补完。

**超长行的后半截能捞回来了（有界字节续读）。**`read_file` 按行翻页，只有一种
情况这样翻**会丢字节**：某一行本身就比 `max_bytes` 长（压缩过的 JS、一行导出
的 JSON）。这时这一页只给了前半截，`next_start_line` 指向下一行，中间那段谁都
没读到；翻到文件末尾 `next_start_line` 变成 `null`，看着就像全读完了——方案 D
说的"不把翻页结束当作全文已读"就是这个。

现在那一页多回 `skipped_bytes`（按行翻页会跳过多少）和 `next_start_byte`
（从哪个字节接着读）。`read_file` 同时收 `start_byte`，给了就按字节读，回
`read_mode="bytes"`。这一路**不数行**，所以 `start_line` / `total_lines` 一律
是 `null`——不是 0，是"没算"；要数行就得从文件头再扫一遍，而这个入口存在的理由
正是不去扫。

两个边界：偏移落在 UTF-8 字符中间时报 `INVALID_ARGUMENT` 并指回上一次返回的
`next_start_byte`（那个一定在边界上），而不是回乱码或者赖文件编码；偏移超过
文件末尾也报错，而不是回空串装作读完了。实现上多跟了两个绝对偏移，其中"被截
的那一行到哪儿结束"可能跨好几个 64 KiB 读取块，测试里有一条 20 万字节的单行
专门压这条路径。

**截断的搜索不再冒充全项目总数。**方案 D 里那句"不能用
`total_matches=当前返回数量` 暗示全项目总数"一直没做。现在拆成两个字段：
`returned_matches` 是这一页给了几条，`total_matches` 只有在没截断时才是数字，
**截断之后是 `null`**——扫描提前停了，全项目有多少条就是不知道。

顺带把遍历顺序定下来（`sort_by_file_name`，`search_text` 和 `list_files` 都
改了）：readdir 的顺序是文件系统说了算的，macOS 和 Linux 不一样，同一个查询
两次跑可能给出不同的 100 条，那"缩小范围再搜一次"这句建议就没法照做。

**续查游标仍然没做，这是选择不是遗漏。**方案 D 的原话是"提供稳定续查游标
**或**明确的细化查询建议"，二选一；U4 选的是建议那一支。做游标要额外付出：
把"从第几行接着搜"穿进三个搜索函数（否则游标文件里被跳过的匹配仍然占
`max_results` 名额，翻页会停在原地）、定义游标失效（文件没了）怎么报。而代码
...
**远端能力代次也做了，跨仓 X10 到此闭合。**`REMOTE_TOOL_UNSUPPORTED` 的话一直是
"那台机器上的 ccnm 太老，升级它"，却从没说过对面**现在**是哪一版——于是没法判断
"我升过了吗"，也没法判断"是不是连错机器了"。现在这条拒绝会报出对面握手时的
`serverInfo`（名字和版本）、工具数，和一份工具表摘要，`details.remote_capabilities`
里也有结构化的一份。

不另造版本号：评审原话是"能力代次不用另造——`serverInfo.version` 加 `tools/list`
就是"。摘要把工具名和各自的参数名**排序之后**再哈希——`HashMap` 的遍历顺序每个
进程都不一样，不排序的话同一份工具表每次算出的数都不同，读的人会以为对面换了版本。
对面没报 `serverInfo` 就是 `unknown`，不编。

只有这一种错带这份依据：别的错发生在核对之前，那时还没有依据可言。

一个实现上的坑：`RemoteGeneration` 直接摊进 `PeerError` 会让这个枚举涨到 128 字节，
而它是所有远端调用的 `Err` 类型——clippy 的 `result_large_err` 当场拦下。装箱解决，
错误路径是冷的，多一次分配无所谓。
