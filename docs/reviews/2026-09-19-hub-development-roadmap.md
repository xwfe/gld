...
日期：2026-09-19。状态：**历史设计路线，部分已实施，不是当前逐项待办。**
2026-09-25 对账：单服务、最小 grant、任务验收和运行记录均已有实现；
L0–L6 的当前归并与排队以 [task_plan.md](../../task_plan.md) 为准。
...
本文件保留提出路线时的依据，不作为当前逐项待办。L0 的 U0–U5 已有验收；G1–G3、
用户级 Skills 和本机 MCP 已实施。L1 的单服务入口已收口，但项目级授权仍缺；L2–L5
不能因存在通用执行和 MCP 转发就标为完整生命周期交付。当前缺口与验收顺序见
[本次审查](2026-09-23-lifecycle-and-docs-audit.md)，使用边界见[生命周期指南](../project-lifecycle.md)。

下面“先完成授权再删除单项目服务”的先后顺序已由 [RFC-0004](../rfc/0004-one-service-many-projects.md)
替代；不重新恢复旧双入口，也不重新立项已明确不做的 PTY。

## 1. 原始结论与边界（2026-09-19）
...
**S2 和 S3 后来被改掉了（2026-09-20 补记）。** 上表是 2026-09-19 审查当时的事实，
原样保留，但照着它推导现状会推错，所以在这里说明：

- **S3 的「每个 `ToolContext` 构造自己的 `SessionStore`」不再成立**（`d947b90`）。
  会话表归 `tools/workspace_runtime.rs`，按「目录 + 调用方主体」分——同一个主体
  换哪个入口进来都是同一张表，换个主体就完全看不见。主体是 `tools/caller.rs` 的
  `Caller`。所以工具内核现在有两个执行入口：`call_tool`（本机操作员）和
  `call_tool_as`（带主体），新的传输层必须用后者。
- **S2 的「配置指纹变化时替换上下文并终止旧会话」只剩前半句**（同一提交）。
  `context_for` 照旧按指纹重建上下文，但**不再终止会话**：表不跟上下文走之后，
...
| 交互 CLI/调试 | 有 stdin/session，但 tty 不等于 PTY | EOF/初始输入已修（U4）；**真 PTY 用户 2026-09-21 定了不做**，debugger 可按需独立支持 |
...
| **L2 日常开发执行** | Execution Job、stdin/EOF、项目任务发现、环境诊断、最小 artifact 和验证证据；吸收 U3–U5 对应项。**不含 PTY**（2026-09-21 用户定了不做）。 | 测试/编译可断开后观察；dev server 可就绪/查询/停止；验证绑定代码快照；不因重连重复执行有副作用的操作。 |
