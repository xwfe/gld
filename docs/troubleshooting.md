...
| `read_output` 说 `unknown`、`command_ok` 是 null | gld 没来得及记下结局就没了（守护进程被 `kill -9`、崩溃、断电） | 命令可能还作为孤儿在跑：回包的 `pid_in_use` 是 true 就先 `pgrep -l -g <pid>` 看看是不是它，是的话自己决定要不要停（gld 不替你杀，那个号可能已经给了别的程序）；false 就是已经没了。结局不明，别直接重跑有副作用的命令 |
