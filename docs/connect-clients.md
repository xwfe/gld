...
- `gld restart`、`gld stop` + `gld start`、重启电脑、守护进程重启
- 改工具集、改项目目录、改工作区名（改名后客户端重连一次才显示新名字）
- 固定域名 / 固定子域名下重启隧道
...
| `gld ws set shared-secrets=true` / `=false` | 签名密钥换了来源，效果同上 |
| 超过 90 天没用过 | 刷新令牌也过期了 |
...
| 操作 | 地址怎么变的 |
| --- | --- |
| 用 `--tunnel cf` 临时地址，隧道重启 | 每次都换一个 `xxx.trycloudflare.com` |
| `gld upgrade --tunnel cf:新域名`、换 frp 子域名 | 你自己换的 |
| `gld ws set global-gateway=true` / `=false` | 在 `https://自己的域名/mcp` 和 `https://入口域名/w/<id>/mcp` 之间切 |
| 全局入口用 `--tunnel cf` 后重启 | 入口地址变了，走它的工作区全受影响 |
| `gld destroy` 后重新登记 | 工作区 id 换了，全局入口路径跟着换 |

**所以长期用就别用临时地址。** 固定域名（`cf:mcp.example.com`）、frp 固定子域名、
自建反代这三种地址不变，上面整张表都碰不到。

改端口（`gld ws set port=…`）不在此列：公网地址没变，连接器照常能用，
...
**客户端里显示的名字就是工作区名。** 接了三个项目，MCP 客户端的服务器列表里
就是三个不同的条目（`api`、`web`、`docs`），不用靠端口号认人。想改就改工作区名：

```bash
gld ws set name=api      # 改完自动重启；客户端那边重连一次才会看到新名字
```

AI 调 `server_info` 拿到的也是这个名字——它问的就是"我现在在哪个项目里"。
