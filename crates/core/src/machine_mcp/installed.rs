//! 这台机器上已经装好的 MCP server：直接读 Claude Code 和 Codex 自己的配置。
//!
//! 只读这两个文件，不改，也不让用户在 gld 里再抄一份清单——抄一份迟早和原件
//! 对不上。gld 自己只记"开了哪几个"（见 [`super`]）。
//!
//! | 读哪儿 | 读什么 |
//! | --- | --- |
//! | `~/.claude.json` | 顶层 `mcpServers`（Claude Code 的 user 级） |
//! | `$CODEX_HOME/config.toml`，没设就是 `~/.codex/config.toml` | `[mcp_servers.*]` |
//!
//! 项目级的（`~/.claude.json` 里 `projects.<路径>.mcpServers`、项目里的
//! `.mcp.json`）不读：它们只在那个项目里生效，算不上"装在这台机器上"。
//!
//! **同名时 `~/.claude.json` 的那份生效**，Codex 那份进 [`Installed::shadowed`]。
//! 没有哪边更权威，只是得定一个，并且让 `gld mcp ls` 说出来。
//!
//! 值里的 `${VAR}` / `${VAR:-默认值}` 照 Claude Code 的规矩展开（只对
//! `~/.claude.json` 的条目；Codex 不展开，它用 `env_vars`、
//! `bearer_token_env_var` 这些键从环境里取）。`~` 不展开：原生客户端也不展开，
//! 由 server 自己处理（Filesystem 就是这么做的）。
//!
//! 这个文件只用标准库、serde_json 和 toml，不碰 gld 的任何类型：v4 第 3 步
//! ccnm 也要读这两份配置，到那时整个搬进 toexec。
...
pub enum Transport {
    ...
    /// 老的 HTTP+SSE 传输（2024-11-05）。读得出来，gld 不支持连它。
    Sse {
        url: String,
        headers: BTreeMap<String, String>,
    },
...
pub struct Server {
    ...
    /// 来源配置里把它关了（Codex 的 `enabled = false`，`~/.claude.json` 里的
    /// `disabled: true` / `enabled: false`）。只用来提示：gld 开不开看它自己的
    /// 名单，操作员在 gld 里点名开了就是开了。
...
pub struct Installed {
    ...
    /// 同名被 `~/.claude.json` 那份盖掉的 Codex 条目。
...
/// 读两份配置。文件不存在就是没装，不算问题；读不了、解析不了的记进
...
pub fn read(places: &Places, env: &dyn Fn(&str) -> Option<String>) -> Installed {
    ...
    let claude = read_claude(&places.claude, env, &mut problems);
    let codex = read_codex(&places.codex, env, &mut problems);

    let mut servers = claude;
...
fn read_claude(
    ...
    let Some(text) = read_file(path, Source::Claude, problems) else {
        return Vec::new();
    };
    ...
    for (name, entry) in entries {
        match claude_server(name, entry, env) {
