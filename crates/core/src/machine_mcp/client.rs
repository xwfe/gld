...
//!
//! 只做 gld 转发要用的这几件：initialize、tools/list（带翻页）、tools/call。
//! server 反过来问的，只答 `ping`，别的回"没有这个方法"——gld 没声明
//! sampling、roots、elicitation，按协议 server 本来就不该问。
//!
//! 跟 `bridge::peer::Peer` 分开写，因为对面不一样：那边是冻结了协议版本的
//! ccnm，版本不对就该停；这边是谁都可能写的 server，实测本机装的有说
//! 2025-06-18 的，也有还停在 2024-11-05 的（`@modelcontextprotocol/server-github`、
//! `server-puppeteer`），两个都得接。stdout 上混进一行不是 JSON 的日志，
//! 官方 TypeScript SDK 是报个错接着读，这里也跳过去接着读。
...
impl Client {
    /// 握手。`timeout` 包括进程冷启动——`npx -y` 第一次要下载包。
    pub fn connect(transport: Box<dyn Transport>, timeout: Duration) -> Result<Client, Error> {
        ...
        let result = client.request(
            ...
            json!({
                ...
                "capabilities": {},
                "clientInfo": { "name": "gld", "version": env!("CARGO_PKG_VERSION") }
            }),
            timeout,
        )?;
        let version = result
...
pub(crate) mod tests {
    ...
    impl Scripted {
        pub(crate) fn new(answer: impl FnMut(&Value) -> Vec<String> + Send + 'static) -> Scripted {
            Scripted {
    ...
    pub(crate) fn reply(request: &Value, result: Value) -> String {
        json!({ "jsonrpc": "2.0", "id": request["id"], "result": result }).to_string()
    }

    /// 一个最普通的 server：握手回 `version`，两个工具，调用回显参数。
    pub(crate) fn plain_server(version: &'static str) -> Scripted {
        Scripted::new(
    ...
    fn an_older_protocol_version_is_accepted_and_the_server_is_described() {
        let client = Client::connect(Box::new(plain_server("2024-11-05")), QUICK).unwrap();
    ...
    fn a_version_nobody_knows_is_refused() {
        let Err(error) = Client::connect(Box::new(plain_server("1999-01-01")), QUICK) else {
            panic!("should refuse");
        };
    ...
    fn log_lines_pings_and_notifications_do_not_get_in_the_way() {
        ...
        Client::connect(Box::new(server), QUICK).expect("handshake");
    ...
    fn every_page_of_tools_is_read() {
        ...
        let mut client = Client::connect(Box::new(server), QUICK).unwrap();
    ...
    fn a_json_rpc_error_keeps_the_connection_and_a_hang_up_does_not() {
        let server = Scripted::new(|request| match request["method"].as_str().unwrap_or("") {
            "initialize" => vec![reply(request, json!({ "protocolVersion": "2025-06-18" }))],
            "tools/call" if request["params"]["name"] == json!("bad") => vec![json!({
